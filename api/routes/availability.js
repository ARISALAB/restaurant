  ...
  try {
    ...
    if (!merchant_id || !start_time_sec || !end_time_sec) {
      return res.status(400).json({
        error: {
          code: 400,
          message: 'merchant_id, start_time_sec, end_time_sec are required',
...
function generateSlots(startDate, endDate, existingBookings, shopConfig) {
  ...
  while (current <= endDate) {
    ...
    if (hour >= OPEN_HOUR && hour < CLOSE_HOUR) {
      ...
      // ✅ ΔΙΟΡΘΩΣΗ: Χρησιμοποιούμε getHours() και getMinutes() σωστά
    ...
    // Αν ξεπεράσαμε CLOSE_HOUR, πάμε στην επόμενη μέρα
