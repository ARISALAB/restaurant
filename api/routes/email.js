 ...
 * Email επιβεβαίωσης στον πελάτη με κουμπί ακύρωσης
 */
router.post('/notify-customer', async (req, res) => {
  try {
    const { shopId, bookingId, cancelToken } = req.body;
    ...
    const cancelUrl = `https://tablereserve.gr/cancel.html?token=${cancelToken}&shop=${shopId}&id=${bookingId}`;
...
<body style="font-family:Arial,sans-serif;background:#f0f2f5;margin:0;padding:20px;">
  <div style="max-width:580px;margin:0 auto;">
    ...
    <div style="background:#2563eb;border-radius:12px 12px 0 0;padding:28px 32px;text-align:center;">
      ...
      <h1 style="color:#fff;margin:0;font-size:20px;font-weight:700;">Επιβεβαίωση Κράτησης</h1>
      <p style="color:rgba(255,255,255,0.8);margin:6px 0 0;font-size:14px;">${shopName}</p>
    </div>

    <div style="background:#fff;padding:28px 32px;">
      <p style="color:#1e293b;font-size:15px;margin-bottom:4px;">Αγαπητέ/ή <strong>${booking.name}</strong>,</p>
      <p style="color:#64748b;font-size:14px;margin-bottom:24px;">Η κράτησή σας επιβεβαιώθηκε με επιτυχία!</p>

      <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:16px;margin-bottom:20px;text-align:center;">
        <p style="margin:0;color:#1d4ed8;font-size:22px;font-weight:700;">${booking.date} &bull; ${booking.time}</p>
        <p style="margin:4px 0 0;color:#3b82f6;font-size:14px;">${booking.guests} άτομα &bull; ${shopName}</p>
      </div>

      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        <tr>
          <td style="padding:10px 12px;background:#f8fafc;border:1px solid #e2e8f0;font-weight:600;color:#64748b;font-size:13px;width:40%;">📍 Χώρος</td>
          <td style="padding:10px 12px;background:#fff;border:1px solid #e2e8f0;color:#1e293b;font-size:14px;">${booking.location || 'Δεν έχω προτίμηση'}</td>
        </tr>
        ${booking.occasion ? `
        <tr>
          <td style="padding:10px 12px;background:#f8fafc;border:1px solid #e2e8f0;font-weight:600;color:#64748b;font-size:13px;">🎉 Γεγονός</td>
          <td style="padding:10px 12px;background:#fff;border:1px solid #e2e8f0;color:#1e293b;font-size:14px;">${booking.occasion}</td>
        </tr>` : ''}
        ${booking.comments ? `
        <tr>
          <td style="padding:10px 12px;background:#f8fafc;border:1px solid #e2e8f0;font-weight:600;color:#64748b;font-size:13px;">💬 Αιτήματα</td>
          <td style="padding:10px 12px;background:#fff;border:1px solid #e2e8f0;color:#1e293b;font-size:14px;">${booking.comments}</td>
        </tr>` : ''}
      </table>

      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:20px;text-align:center;">
        <p style="color:#991b1b;font-size:13px;font-weight:600;margin:0 0 12px;">Θέλετε να ακυρώσετε την κράτησή σας;</p>
        <a href="${cancelUrl}"
        ...
        <p style="color:#94a3b8;font-size:11px;margin:12px 0 0;">
          Η ακύρωση επιτρέπεται έως 2 ώρες πριν την κράτηση
        </p>
      </div>
    </div>
    ...
    console.log(`[notify-customer] Email εστάλη στο ${booking.email}`);
